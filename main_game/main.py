...
class GameWidget(Widget):
    def __init__(self, **kwargs):
        ...
        with self.canvas:
            self._score_instruction = Rectangle(texture=self._score_label.texture,
                                                pos=(0, 550), size=self._score_label.texture.size)

        self.keysPressed = set()
        self._entities = set()

        Clock.schedule_interval(self._on_frame, 0)  # 0 means every frame

        # play sound
        self.sound = SoundLoader.load("assets/sound.wav")
        self.sound.play()
    ...
    def colliding_entities(self, entity):
        ...
        for e in self._entities:
            if self.collides(e, entity) and e != entity:  # check for self collision too??
                result.add(e)
        return result
...
class Entity():
    def __init__(self):
        ...
        self._instruction = Rectangle(
            source=self._source, pos=self._pos, size=self._size)
...
class Bullet(Entity):
    def __init__(self, pos, speed=300):
        super().__init__()
        sound = SoundLoader.load("assets/gun.wav")
        sound.play()
        self._speed = speed
        self.pos = pos
        self.size = (30, 30)
    ...
    def move_step(self, sender, dt):
        # check for collision/out of bounce
...
class Player(Entity):
    def __init__(self):
        super().__init__()
        self.source = "assets/spaceship.png"
        game.bind(on_frame=self.move_step)
        self.shoot_event = Clock.schedule_interval(self.shoot_step, 0.1)
        self.pos = (400, 0)
    ...
    # we have seperate fn for shooting to avoid too many  bullets
